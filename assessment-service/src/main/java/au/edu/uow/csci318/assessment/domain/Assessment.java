...
@Entity @Table(name="assessments",uniqueConstraints=@UniqueConstraint(columnNames={"owner_id","subject_id","title"}))
public class Assessment{
 public enum Status{INCOMPLETE,COMPLETED} public enum Priority{LOW,MEDIUM,HIGH}
 @Id private UUID id;@Column(name="owner_id",nullable=false)private UUID ownerId;@Column(nullable=false)private UUID subjectId;@Column(nullable=false)private String title;private String type;private Double weighting;private LocalDate dueDate;private Integer dueWeek;private String description;private Integer estimatedMinutes;@Enumerated(EnumType.STRING)private Priority priority;@Enumerated(EnumType.STRING)private Status status;@Column(nullable=false)private Instant updatedAt;
 protected Assessment(){}
 public Assessment(UUID ownerId,UUID subjectId,String title,String type,Double weighting,LocalDate dueDate,Integer dueWeek,String description,Integer estimatedMinutes,Priority priority){id=UUID.randomUUID();this.ownerId=Objects.requireNonNull(ownerId);this.subjectId=Objects.requireNonNull(subjectId);this.title=require(title);this.type=type;changeWeighting(weighting);changeDeadline(dueDate,dueWeek);this.description=description;changeEstimatedWorkload(estimatedMinutes);this.priority=priority==null?Priority.MEDIUM:priority;status=Status.INCOMPLETE;updatedAt=Instant.now();}
 public void changeDeadline(LocalDate date,Integer week){if(date!=null&&date.isBefore(LocalDate.of(2000,1,1)))throw new IllegalArgumentException("Due date is invalid");if(week!=null&&(week<1||week>52))throw new IllegalArgumentException("Due week must be between 1 and 52");dueDate=date;dueWeek=week;touch();}
