...
import au.edu.uow.csci318.subject.infrastructure.IdentityClient.IdentityServiceException;
import au.edu.uow.csci318.subject.infrastructure.IdentityClient.UnauthorizedIdentityException;
...
public class ApiExceptionHandler {
  ...
  ResponseEntity<ErrorBody> bad(RuntimeException e, HttpServletRequest r) {
    return body(HttpStatus.BAD_REQUEST, e, r, null);
  }

  @ExceptionHandler(NoSuchElementException.class)
  ResponseEntity<ErrorBody> missing(RuntimeException e, HttpServletRequest r) {
    return body(HttpStatus.NOT_FOUND, e, r, null);
  }

  @ExceptionHandler(ServiceDependencyException.class)
  ResponseEntity<ErrorBody> unavailable(RuntimeException e, HttpServletRequest r) {
    return body(HttpStatus.SERVICE_UNAVAILABLE, e, r, null);
  }
  ...
  ResponseEntity<ErrorBody> unauthorized(RuntimeException e, HttpServletRequest r) {
    return body(HttpStatus.UNAUTHORIZED, e, r, null);
  }

  @ExceptionHandler(MethodArgumentNotValidException.class)
  ResponseEntity<ErrorBody> invalid(MethodArgumentNotValidException e, HttpServletRequest r) {
    Map<String, String> v = new LinkedHashMap<>();
    e.getBindingResult().getFieldErrors().forEach(x -> v.put(x.getField(), x.getDefaultMessage()));
    return body(HttpStatus.BAD_REQUEST, new IllegalArgumentException("Validation failed"), r, v);
  }

  private ResponseEntity<ErrorBody> body(
      HttpStatus s, RuntimeException e, HttpServletRequest r, Map<String, String> v) {
