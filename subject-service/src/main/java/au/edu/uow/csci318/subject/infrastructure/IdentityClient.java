...
public class IdentityClient {
    ...
    public IdentityClient(RestClient.Builder builder, @Value("${services.account-url}") String url) {
        accounts = builder.baseUrl(url).build();
    }
    public UUID require(String authorization) {
        ...
        try {
            Identity identity = accounts.get().uri("/api/auth/validate").header("Authorization", authorization)
