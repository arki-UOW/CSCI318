...
import au.edu.uow.csci318.subject.application.SubjectAssessmentGateway;
...
@Component
class AssessmentImportClient implements SubjectAssessmentGateway {
  ...
  public void importAssessments(
    ...
    try {
      ...
      throw new SubjectApplicationService.ServiceDependencyException(
          "Assessment Service is unavailable. Confirmation is still pending; try again when it is"
              + " running.",
          exception);
    } catch (ResourceAccessException exception) {
      throw new SubjectApplicationService.ServiceDependencyException(
          "Assessment Service could not be reached. Confirmation is still pending; try again when"
              + " it is running.",
          exception);
    }
  }
