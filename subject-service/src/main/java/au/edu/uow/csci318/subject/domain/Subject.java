...
public class Subject {
  ...
  public Subject(UUID ownerId, String code, String name, Integer creditPoints, int target) {
    ...
    changeWeeklyStudyTarget(target);
  }

  private static String normaliseCode(String value) {
    String v = requireText(value, "Subject code").toUpperCase();
    if (!v.matches("[A-Z]{2,8}[0-9]{3,4}"))
      throw new IllegalArgumentException("Subject code must look like CSCI318");
    return v;
  }
  ...
  public void changeWeeklyStudyTarget(int minutes) {
  ...
  public int getWeeklyStudyTargetMinutes() {
    return weeklyStudyTargetMinutes;
  }
}
