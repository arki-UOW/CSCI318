...
public final class PlanningDtos {
  ...
  public record PlanItem(
      ...
      int repetitionStage) {
    public PlanItem(
        LocalDate date, UUID subjectId, UUID assessmentId, String title, int allocatedMinutes) {
  ...
  public record AvailabilityChatRequest(
      @NotBlank String message,
      @NotNull LocalDate weekStart,
      @NotNull Map<LocalDate, List<@Valid TimeSlot>> availability) {}

  public record AvailabilityChatResponse(
      String reply,
      Map<LocalDate, List<TimeSlot>> availability,
  ...
  public record WorkloadSummary(
      ...
      String status,
      int estimatedMinutesDueWithinSevenDays) {
    public WorkloadSummary(
        int incomplete, int dueWeek, int dueSeven, int minutes, int high, String status) {
      this(incomplete, dueWeek, dueSeven, minutes, high, status, 0);
    }
  }
  ...
  public record StreamSnapshot(ThisWeek week, StreamStatus stream) {}

  public record StudyProgress(
      UUID subjectId, int studiedMinutes, int targetMinutes, int remainingMinutes, String state) {}

  public record ThisWeek(
      LocalDate weekStart,
      LocalDate weekEnd,
      List<AssessmentView> dueThisWeek,
      List<AssessmentView> upcoming,
      WorkloadSummary workload,
      List<StudyProgress> studyProgress,
