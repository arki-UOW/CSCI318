...
class CalendarApplicationServiceTest {
    ...
    void completionCreatesFirstSpacedReviewForTomorrow() {
        ...
        CalendarEntry original = new CalendarEntry(ownerId, UUID.randomUUID(), null, null, null,
                "Review lecture notes", "", STUDY_SESSION, start, start.plusMinutes(45), MANUAL, true, 0);
        when(entries.findByIdAndOwnerId(original.getId(), ownerId)).thenReturn(Optional.of(original));
        when(entries.save(any(CalendarEntry.class))).thenAnswer(invocation -> invocation.getArgument(0));

        var result = new CalendarApplicationService(entries).complete(ownerId, original.getId());
        ...
        assertEquals(45, java.time.Duration.between(result.nextReview().startAt(),
                result.nextReview().endAt()).toMinutes());
