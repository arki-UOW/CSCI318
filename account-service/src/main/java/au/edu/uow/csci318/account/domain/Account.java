...
public class Account {
  ...
  private boolean timezoneAutomatic = true;
  ...
  @Column(length = 512)
  private String navigationOrder;
  ...
  public void updateProfile(
      String displayName, String institution, String course, String studyGoal, String timezone) {
    this.displayName = require(displayName, "Display name");
    this.institution = clean(institution);
    this.course = clean(course);
    this.studyGoal = clean(studyGoal);
    this.timezone = require(timezone, "Timezone");
    this.timezoneAutomatic = false;
  }

  public void useDetectedTimezone(String timezone) {
    if (timezoneAutomatic) this.timezone = require(timezone, "Timezone");
  }

  public void updateTheme(
      String primary, String accent, String background, String surface, String text) {
  ...
  public void updateTheme(
    ...
    this.navigationColor =
        colour(
            navigation == null ? getNavigationColor() : navigation, "Tab hover and active colour");
  }
  ...
  public void updateProfilePicture(String picture) {
    ...
    if (value.length() > 14_000_000
        || comma < 0
        || !value.substring(0, comma + 1).matches("^data:image/(?:png|jpeg|webp);base64,$")) {
      throw invalidProfilePicture();
    }
    try {
      byte[] decoded = Base64.getDecoder().decode(value.substring(comma + 1));
      if (decoded.length > 10 * 1024 * 1024) throw invalidProfilePicture();
  ...
  private static IllegalArgumentException invalidProfilePicture() {
    return new IllegalArgumentException(
        "Profile picture must be a PNG, JPEG or WebP image up to 10 MB");
  }
  ...
  private static String colour(String value, String label) {
    ...
    if (!colour.matches("#[0-9a-f]{6}"))
      throw new IllegalArgumentException(label + " must be a six-digit hex colour");
    return colour;
  }
  ...
  public String getNavigationColor() {
    return navigationColor == null ? "#2a5745" : navigationColor;
  }
