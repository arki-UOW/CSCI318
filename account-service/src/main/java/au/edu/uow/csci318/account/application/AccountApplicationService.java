...
public class AccountApplicationService {
  private static final Set<String> NAVIGATION_VIEWS =
      Set.of("dashboard", "subjects", "assessments", "plan", "calendar", "activity");
  ...
  public SessionResponse register(RegisterRequest request) {
    ...
    if (accounts.existsByUsername(username))
      throw new DuplicateUsernameException("That username is already in use");
    ...
    Account account =
        accounts.save(
            new Account(username, passwords.encode(request.password()), displayName, timezone));
  ...
  public SessionResponse login(LoginRequest request) {
    Account account =
        accounts
            .findByUsername(Account.normaliseUsername(request.username()))
  ...
  @Transactional
  public AccountResponse updateDetectedTimezone(
      String authorization, DetectedTimezoneRequest request) {
    String timezone = ZoneId.of(request.timezone()).getId();
    Account account = requireAccount(authorization);
    account.useDetectedTimezone(timezone);
    return response(accounts.save(account));
  }
  ...
  public AccountResponse updateNavigation(String authorization, NavigationUpdateRequest request) {
    ...
    if (order.size() != NAVIGATION_VIEWS.size() || !order.equals(NAVIGATION_VIEWS)) {
      throw new IllegalArgumentException(
          "Navigation order must contain every visible tab exactly once");
    }
    Account account = requireAccount(authorization);
  ...
  private Account requireAccount(String authorization) {
    ...
    AccountSession session =
        sessions
            ...
            .orElseThrow(
                () -> new UnauthorizedException("Your session is invalid. Sign in again."));
  ...
  private AccountResponse response(Account account) {
    return new AccountResponse(
        ...
        account.getCourse(),
        account.getStudyGoal(),
        account.getTimezone(),
        account.isTimezoneAutomatic(),
        account.getPrimaryColor(),
        account.getAccentColor(),
        account.getBackgroundColor(),
        account.getSurfaceColor(),
        account.getTextColor(),
        account.getNavigationColor(),
        account.getProfilePicture(),
        List.of(account.getNavigationOrder().split(",")),
